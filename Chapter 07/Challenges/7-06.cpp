...
void getInfoFromFile(char array[][COLUMNS], string file_name){//khoi tao ham getInfoFromFile gom mang ky tu, bien ky tu
    ifstream inputFile;
...
void displayHighest(const char array[][COLUMNS]){//khoi tao ham displayHighest gom mang ky tu hang
    ...
    for(int i = 1; i < ROWS; i++){//khoi tao vong lap duyet theo hang
        if(total_rainy_days[i] > highest_rainy_day){//neu gia tri trong ngoac thoa man thi thuc hien cac cau lenh 
            highest_rainy_day = total_rainy_days[i];
            highest_rainy_day_name = months[i];
        } 
        else if(total_rainy_days[i] == highest_rainy_day){//neu gia tri trong ngoac thoa man thi thuc hien cac cau lenh 
            highest_rainy_day = total_rainy_days[i];
            highest_rainy_day_name += ", ";
            highest_rainy_day_name += months[i];
        }
    }
    //xuat ra amn hinh ket qua
